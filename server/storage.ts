...
export class MemStorage implements IStorage {
    ...
    async getDevices(): Promise<DeviceWithUser[]> {
        ...
        return deviceList.map((device) => ({
            ...
            assignedUser: device.assignedTo
                ? this.users.get(device.assignedTo)
                : undefined,
        }));
    }
    ...
    async getRequests(): Promise<RequestWithUser[]> {
        ...
        return requestList.map((request) => ({
            ...
            assignedDevice: request.assignedDeviceId
                ? this.devices.get(request.assignedDeviceId)
                : undefined,
            approver: request.approvedBy
                ? this.users.get(request.approvedBy)
                : undefined,
        }));
    }
    ...
    async getDeviceLogs(deviceId: string): Promise<DeviceLog[]> {
        return Array.from(this.deviceLogs.values()).filter(
    ...
    async getStats() {
        ...
        return {
            ...
            availableDevices: deviceList.filter((d) => d.status === "available")
                .length,
            assignedDevices: deviceList.filter((d) => d.status === "assigned")
                .length,
            maintenanceDevices: deviceList.filter(
...
// Use MongoStorage for persistent data
